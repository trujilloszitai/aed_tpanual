...
void generarDNI(char* dni) {
    ...
    snprintf(dni, 9, "%08d", numDNI);  // Convertir el número en una cadena de 8 caracteres
}

// Función para generar un número de teléfono aleatorio
void generarTelefono(char* telefono) {
    snprintf(telefono, 21, "+54 9 %02d-%04d-%04d", rand() % 100, rand() % 10000, rand() % 10000);
}

// Función para generar un array de pacientes
void generarPacientes(Paciente pacientes[], int N) {
    const char* nombres[] = {"Juan", "Ana", "Pedro", "Lucia", "Carlos", "Maria", "Luis", "Elena", "Miguel", "Laura"};
    const char* apellidos[] = {"Perez", "Gomez", "Fernandez", "Rodriguez", "Luna", "Martinez", "Sanchez", "Diaz", "Silva", "Morales"};
...
void generarTurnos(infoT turnos[], int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        // Genera una hora aleatoria en formato 24 horas con intervalos de 15 minutos
        ...
        nuevoTurno.turno.idPaciente = 1 + rand() % 100; // ID del paciente entre 1 y 100
